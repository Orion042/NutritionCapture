...
class GeminiUtils {
    ...
    suspend fun getGeminiProResponseFromBitmap(bitmap: Bitmap) : String? {
        ...
        val prompt = """画像から以下の質問に対する回答を生成してください。回答言語は日本語にしてください。
                        1. 画像に映っている食べ物を指定した形式のdishesに入れてください。食べ物は複数ある可能性もあります。画像の隅々までチェックしてください。また、食べ物ではない場合は食べ物がみつかりませんでした。としてください。
                        2. 食べ物に含まれている食材を指定した形式のingredientsに入れてください。
                        3. 食べ物の推定カロリーを指定した形式のcalorieに入れてください。数値は厳しく点数付けしてください。数値のみにしてください。
                        指定形式
